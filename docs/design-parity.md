...
| Reference | Verdict |
|---|---|
| `foundations/colors.html` | PASS — enforced by `src/styles/tokens.test.ts`. Severity ramp used only for scores/trend line, accent only for interaction, dose gold only on dose markers, danger only on error/destructive states. |
...
| `components/date-navigator.html` | PASS — 36px arrow buttons, centered subtitle-weight header, fixed Monday–Sunday 7-day strip spanning full width (`justify-content: space-between`) that only moves on week-arrow navigation, weekday+day-of-month per pill, dashed+no-dot = unknown, solid+dot = recorded, filled accent = selected day, accent outline ring = today, dimmed+disabled = future day. Header reads `Month Year` for a week inside one month, `Month1 - Month2 Year` crossing a month boundary, `Month1 Year1 - Month2 Year2` crossing a year boundary (`weekRangeLabel` in `src/domain/date.ts`). |
...
| `components/gap-scatter.html` | No reference file exists for this component — it was added after the last parity pass with no corresponding mockup. Styled by composing existing tokens and conventions from `trend-chart.html` (severity-3 fill, dashed `--baseline-line` reference line, same legend/caption layout) rather than matching a dedicated reference. Flagged here as a deliberate non-reference addition, same treatment given to the trend-chart per-point-coloring simplification below. |
| `components/window-toggle.html` | PASS — pill container, `accent-muted` fill + `accent` text on the active option. |
| `components/export-action.html` | PASS — title + note on the left, secondary button on the right; the import row reuses that same card shape, adding a micro-sized status line below the note (`--text-secondary` on success, `--danger` on a refused file). |
