...
| `components/date-navigator.html` | PASS — 36px arrow buttons, centered subtitle-weight month/year header, fixed Monday–Sunday 7-day strip spanning full width (`justify-content: space-between`) that only moves on week-arrow navigation, weekday+day-of-month per pill, dashed+no-dot = unknown, solid+dot = recorded, filled accent = selected day, accent outline ring = today, dimmed+disabled = future day. |
